...
# === 读取矩阵数据 ===
matrix = np.loadtxt('D:/Python/pycharm/nas/1D,2D-histograms/WA-BJ_3Dhist.txt')  # 读取矩阵
...
# === 创建坐标轴 ===
x = np.linspace(-4.93000e-01, 3.0, Nx)
...
v_max = 200  # 用户手动设置 v_max

# === 设置手动的坐标轴范围 ===
x_min = -0.1  # 用户手动设置x轴最小值
x_max = 0.65  # 用户手动设置x轴最大值
y_min = -6.5  # 用户手动设置y轴最小值
y_max = 0.5  # 用户手动设置y轴最大值
...
# # === 定义自定义色图 ===
custom_cmap = LinearSegmentedColormap.from_list("custom_red", ["white", "#DFB6BC", "#fff7e9"])  # 自定义色带-红色黄色
...
plt.xticks(np.linspace(0, 0.6, 5))
plt.yticks(np.linspace(-6, 0, 7))
...
ax.set_xlabel('ΔZ / nm', fontname=x_axis_font, fontsize=font_size)   # 设置x轴标签字体和大小
ax.set_ylabel('Conductance / log(G/G$_0$)', fontname=y_axis_font, fontsize=font_size)   # 设置y轴标签字体和大小

plt.show()
# plt.savefig("2d.png", dpi=300, transparent=True)